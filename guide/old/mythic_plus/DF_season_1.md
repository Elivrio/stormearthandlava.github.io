...
* {{ site.data.affixes.raging }}: Be ready to use your kiting toolkit to support your tank when needed. Consider using {{ site.data.spell.capacitor_totem }} and {{ site.data.spell.sk }} to finish {{ site.data.affixes.enrage }} mobs more safely and quickly.
...
* {{ site.data.affixes.grievous }}: Remember that direct heals remove one stack of {{ site.data.affixes.grievous_wound }}. Support your party members with {{ site.data.spell.healing_surge }} and {{ site.data.spell.ag }} as needed.
* {{ site.data.affixes.quaking }}: Maintain your distance from party members, and be mindful about canceling casts if {{ site.data.affixes.quake }} overlaps with {{ site.data.spell.swg }}.
* {{ site.data.affixes.storming }}: Maintain your distance from mobs to avoid {{ site.data.affixes.storming }}. Use {{ site.data.spell.swg }} as needed in tight spaces or when maneuvering any mechanics.
...
* Use {{ site.data.spell.swg }} to keep uptime while finding your partner.
...
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.aa.monotonous_lecture }}. You can also ignore the cast and just use {{ site.data.spell.tremor_totem }} to dispel its effects.
...
* {{ site.data.dungeon.aa.arcane_ravager }} will jump on a random player with {{ site.data.dungeon.aa.vicious_ambush }} and cast a {{ site.data.dungeon.aa.riftbreath }} frontal. Beware of it at all times and use {{ site.data.spell.swg }} to reposition as needed.

<hr>

### {{ site.data.dungeon.aa.vexamus }}

* Soak the {{ site.data.dungeon.aa.arcane_orbs }} but try to let the {{ site.data.dungeon.aa.oversurge }} debuff fall off before soaking another. Use {{ site.data.spell.as }} at higher stacks.
* Spread with {{ site.data.dungeon.aa.mana_bombs }} and try to drop pools along the walls.
* Beware of {{ site.data.dungeon.aa.arcane_fissure }}, it does a lot of direct damage. Use {{ site.data.spell.as }} as needed. {{ site.data.spell.swg }} will allow you to keep uptime while dodging the swirlies.
...
* Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Pick up wind orbs for 45% haste and a "cast while moving" buff for 20s. Do not overlap the orb buff with {{ site.data.spell.swg }}.
* Try to be at full health before {{ site.data.dungeon.aa.deafening_screech }} or use {{ site.data.spell.as }}, especially at high {{ site.data.dungeon.aa.sonic_vulnerability }} stacks.
...
* Use {{ site.data.spell.poison_cleansing_totem }} if talented to help the tank with the {{ site.data.dungeon.aa.lasher_toxin }} stacks.

**{{ site.data.dungeon.aa.vile_lasher }}**
* Dodge the big {{ site.data.dungeon.aa.detonation_seeds }} green swirlies. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Consider using {{ site.data.spell.as }} at low health.

<hr>

### {{ site.data.dungeon.aa.overgrown_ancient }}

* The boss will use {{ site.data.dungeon.aa.burst_forth }} after two {{ site.data.dungeon.aa.germinate }} casts. Keep AoE CDs such as {{ site.data.spell.sk }} or {{ site.data.spell.lmt }} to quickly burst down the adds after the second {{ site.data.dungeon.aa.germinate }} cast.
* Move with your group during {{ site.data.dungeon.aa.germinate }} to keep adds stacked for cleave. Use {{ site.data.spell.swg }} to start ramping when a {{ site.data.dungeon.aa.burst_forth }} is coming soon.
* Use CC such as {{ site.data.spell.capacitor_totem }} or {{ site.data.spell.thundershock }} to reduce the group damage when the adds spawn.
* Use {{ site.data.spell.poison_cleansing_totem }} if talented to help the tank with the {{ site.data.dungeon.aa.lasher_toxin }} stacks.
...
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.aa.celestial_shield }} as soon as possible.
...
* Use {{ site.data.spell.swg }} during {{ site.data.dungeon.aa.power_vacuum }} to keep uptime and quickly get out.
...
* Use {{ site.data.spell.wind_shear }} or {{ site.data.spell.purge }} on {{ site.data.dungeon.cos.fortification }}.
...
* Do not pull {{ site.data.dungeon.cos.duskwatch_sentry }} first. It will use {{ site.data.dungeon.cos.throw_torch_cos }} on its first target that will leave a nasty DoT on you. Use {{ site.data.spell.as }} if needed.
* It can be useful to use {{ site.data.spell.hex }} to stop the {{ site.data.dungeon.cos.duskwatch_sentry }} patrols.
...
* They jump on you with {{ site.data.dungeon.cos.mana_fang }} and leave a nasty DoT around you. Stay spreaded and use {{ site.data.spell.as }} if needed.
...
* {{ site.data.dungeon.cos.duskwatch_reinforcement }} will cast {{ site.data.dungeon.cos.subdue }} periodically. It needs to be interrupted at all cost. You can also use {{ site.data.spell.hex }} to CC them far away.
* {{ site.data.dungeon.cos.arcane_lockdown }} is a 90% slow that can either be dispelled or removed by jumping 3 times. You can use {{ site.data.spell.thunderous_paws }} or {{ site.data.spell.spirit_walk }} to instantly dispel it, or use {{ site.data.spell.swg }} to keep uptime while jumping.
...
* {{ site.data.dungeon.cos.eye_storm }} needs to be cancelled with a hard CC ({{ site.data.spell.capacitor_totem }}, {{ site.data.spell.hex }} or {{ site.data.spell.thunderstorm }}) or it will deal lethal damage to your party.

**{{ site.data.dungeon.cos.blazing_imp }}**
* {{ site.data.dungeon.cos.blazing_imp }} often come in large packs and continuously cast {{ site.data.dungeon.cos.firebolt }} or {{ site.data.dungeon.cos.drifting_embers }}. Interrupt, stun and quickly burst them down. Use {{ site.data.spell.as }} if many cast are going through.
...
* {{ site.data.dungeon.cos.impending_doom }} will go on two players. Be ready to use {{ site.data.spell.as }} if needed when the healer doesn't dispel you first.
...
* Stay packed at all time on your tank. Use {{ site.data.spell.swg }} to keep uptime while moving.
...
* Dodge the {{ site.data.dungeon.cos.infernal_eruption }} swirlies then blast the group of {{ site.data.dungeon.cos.blazing_imp }}. Use {{ site.data.spell.capacitor_totem }} if they are about to cast.
...
* Use {{ site.data.spell.capacitor_totem }} or {{ site.data.spell.hex }} to interrupt the {{ site.data.dungeon.cos.hypnosis_bat }} cast.
...
* {{ site.data.dungeon.cos.slicing_maelstrom }} does pulsing damage around {{ site.data.dungeon.cos.advisor_melandrus }} and his clones. You **need** to be 7 yards away from them during the channel. Use {{ site.data.spell.as }} as needed and {{ site.data.spell.ag }} to help the healer.
...
* Remember to use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Dodge the {{ site.data.dungeon.hov.crackling_storm }} swirly and the following tornado. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* {{ site.data.dungeon.hov.etch }} is a channeled beam on a random player. Don't hesitate to use {{ site.data.spell.as }} if you are targeted.
...
* Be ready to instantly {{ site.data.spell.purge }} {{ site.data.dungeon.hov.protective_light }} when they reach 30% HP.
...
* Hide under the dome during {{ site.data.dungeon.hov.eye_of_the_storm }}. This is a great place to use {{ site.data.spell.ag }} and {{ site.data.spell.as }}.
...
* Dodge the orbs of light during {{ site.data.dungeon.hov.sanctify }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* This is a very intense fight healing wise. Be ready to pop {{ site.data.spell.as }}, {{ site.data.spell.ee }} or even {{ site.data.item.potion_rhp }} and {{ site.data.externals.hs }}.
* Hide under the dome during {{ site.data.dungeon.hov.eye_of_the_storm }}. This is a great place to use {{ site.data.spell.ag }} and {{ site.data.spell.as }}.
...
* These mobs are super deadly. They buff themselves while pulled together then jump on ranged players with {{ site.data.dungeon.hov.leap_for_the_throat }}. Stay close to melee but spreaded and use {{ site.data.spell.as }} as required.
...
* Kite the boss if you are targeted by {{ site.data.dungeon.hov.scent_of_blood }}. Be careful not to run into the remaining {{ site.data.dungeon.hov.ebonclaw_worg }} in the area. Use {{ site.data.spell.swg }} to keep uptime while kiting.
...
* He will use {{ site.data.dungeon.hov.wicked_dagger }} on a random party member for huge damage and a 50% healing debuff. In higher keys, track who is targeted and use {{ site.data.spell.as }} if needed.

**{{ site.data.dungeon.hov.king_tor }}**
* Use your CCs to help kiting the Ancestors spawned by {{ site.data.dungeon.hov.call_ancestor }}. {{ site.data.spell.earthbind_totem }}, {{ site.data.spell.capacitor_totem }}, {{ site.data.spell.earthgrab_totem }}, {{ site.data.spell.thunderstorm }}, {{ site.data.spell.thundershock }} and {{ site.data.spell.lightning_lasso }} all work !

### {{ site.data.dungeon.hov.god_king_skovald }}

* Stay spreaded, he will target a random player with {{ site.data.dungeon.hov.felblaze_rush }}, hitting everyone within 6 yards. Use {{ site.data.spell.as }} as needed.
...
* Kite the {{ site.data.dungeon.hov.flame_of_woe }}, if possible with {{ site.data.spell.swg }}. You can spread {{ site.data.spell.fs }} on the {{ site.data.dungeon.hov.flame_of_woe }} for {{ site.data.spell.splinter }} value. Be quick about it because they will burn themselves out with {{ site.data.dungeon.hov.consuming_flame }}.
...
* While running to your assigned rune, you will take heavy damage. Use {{ site.data.spell.as }} to survive. {{ site.data.spell.swg }} paired with {{ site.data.spell.healing_surge }} can be clutch while moving since your healer will also be running to their matching rune.
...
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.rlp.ice_shield }} on the buffed enemy.

**{{ site.data.dungeon.rlp.flashfrost_earthshaper }}**
* Watch your feet and dodge {{ site.data.dungeon.rlp.tectonic_slam }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.

**{{ site.data.dungeon.rlp.primal_juggernaut }}**
* Watch your feet and dodge {{ site.data.dungeon.rlp.excavating_blast }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Dodge the red swirlies created by {{ site.data.dungeon.rlp.steel_barrage }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Move away from the group to drop {{ site.data.dungeon.rlp.chillstorm }}. Use {{ site.data.spell.gust_of_wind }}
to quickly leave the AoE.  Use {{ site.data.spell.swg }} to keep uptime while getting pulled.
* Use {{ site.data.spell.as }} and {{ site.data.spell.ag }} during {{ site.data.dungeon.rlp.frost_overload }} to survive. Use CDs to quickly burst her shield.
...
* You can use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.rlp.burning_ambition }}. But {{ site.data.dungeon.rlp.burning_ambition }} will also damage the caster while it's active so it's not mandatory. Make an educated choice with your group health in mind.

**{{ site.data.dungeon.rlp.primalist_flamedancer }}**
* Use {{ site.data.spell.purge }} on {{ site.data.dungeon.rlp.blaze_of_glory }} as soon as they reach 0 HP. Dodge the red swirlies that spawn while {{ site.data.dungeon.rlp.blaze_of_glory }} is active.

**{{ site.data.dungeon.rlp.blazebound_destroyer }}**
* Use {{ site.data.dungeon.rlp.living_bomb }} on the enemies to bump them and deal some damage. Use {{ site.data.spell.as }} as needed.
* Use {{ site.data.spell.ag }} to help the healer during {{ site.data.dungeon.rlp.inferno }}.

**{{ site.data.dungeon.rlp.thunderhead }}**
* Move fast when targeted by the {{ site.data.dungeon.rlp.storm_breath }} frontal. Use {{ site.data.spell.swg }} to keep uptime while dodging.
* Use {{ site.data.spell.ag }} to help the healer.
* Use {{ site.data.spell.as }} if you get the {{ site.data.dungeon.rlp.rolling_thunder }} debuff.

**{{ site.data.dungeon.rlp.flamegullet }}**
* Move fast when targeted by the {{ site.data.dungeon.rlp.flame_breath }} frontal. Use {{ site.data.spell.swg }} to keep uptime while dodging.
* {{ site.data.dungeon.rlp.molten_blood }} does ramping damage starting at 50% HP. Use {{ site.data.spell.ag }} to help the healer below that threshold.
* Use {{ site.data.spell.as }} as required.
...
* Use {{ site.data.spell.swg }} aggressively, it's very good on this fight and should be used as soon as you need to move for maximum uptime on it.
...
* Use {{ site.data.spell.purge }} on {{ site.data.dungeon.rlp.tempest_barrier }} and they will suicide with {{ site.data.dungeon.rlp.crackling_detonation }}.
* **DO NOT** interrupt {{ site.data.dungeon.rlp.crackling_detonation }}. Just dodge the blue swirlies. Use {{ site.data.spell.swg }} to keep uptime while dodging.

**{{ site.data.dungeon.rlp.tempest_channeler }}**
* Use {{ site.data.spell.purge }} on {{ site.data.dungeon.rlp.primal_thundercloud }} summoned by {{ site.data.dungeon.rlp.summon_primal_thundercloud }}.
* Use {{ site.data.spell.ag }} and {{ site.data.spell.as }} to mitigate {{ site.data.dungeon.rlp.lightning_storm }}.
...
* You can use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.rlp.burning_veins }}. But {{ site.data.dungeon.rlp.burning_veins }} will also damage the caster while it's active so it's not mandatory. Make an educated choice with your Tank health in mind.
...
* You want to use {{ site.data.spell.purge }} on {{ site.data.dungeon.rlp.primal_thundercloud }} **in priority** to remove as many {{ site.data.dungeon.rlp.tempest_barrier }} as possible.
* Use {{ site.data.spell.ag }} and {{ site.data.spell.as }} to mitigate {{ site.data.dungeon.rlp.lightning_storm }}.
...
* Dodge {{ site.data.dungeon.rlp.roaring_firebreath }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Use {{ site.data.spell.as }} if you get the {{ site.data.dungeon.rlp.infernocore }} debuff and try to stand far from the group to drop {{ site.data.dungeon.rlp.flaming_embers }} in a good spot.
* {{ site.data.dungeon.rlp.infernocore }} will go on the whole group during phase 2. Use {{ site.data.spell.ag }} to help your healer.
...
* You can Line of sight to take no damage from {{ site.data.dungeon.sbg.void_pulse }}. Use {{ site.data.spell.as }} if you don't.
* {{ site.data.spell.ag }} is very good to help your healer during {{ site.data.dungeon.sbg.void_pulse }}.
* Use {{ site.data.spell.swg }} to dodge {{ site.data.dungeon.sbg.void_eruptions }} purple swirlies.

**{{ site.data.dungeon.sbg.shadowmoon_loyalist }}**
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.sbg.sinister_focus }}.
...
* Use {{ site.data.spell.ag }} to help healing the damage of {{ site.data.dungeon.sbg.whispers_of_the_dark_star }}. Use {{ site.data.spell.as }} to help yourself if needed.
* Immediately switch on the Defiled Soul when {{ site.data.dungeon.sbg.dark_communion_sbg }} starts. Remember to use {{ site.data.spell.fs }} on both targets. It's a good moment to gain extra value from {{ site.data.spell.splinter }}.
...
* You can use {{ site.data.spell.tremor_totem }} to prevent any charm from {{ site.data.dungeon.sbg.domination }}.
...
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.sbg.incorporeal }}.
...
* Dodge the purple swirlies from {{ site.data.dungeon.sbg.void_devastation }}. Remember to use {{ site.data.spell.swg }} to keep uptime while moving.
* During {{ site.data.dungeon.sbg.soul_steal }}, you will be separated from your healer and take moderate damage. Use {{ site.data.spell.as }} and {{ site.data.item.potion_rhp }} as needed.
...
* Dodge {{ site.data.dungeon.sbg.body_slam }}. Use {{ site.data.spell.swg }} if needed.
...
* Use {{ site.data.spell.fs }} on all three targets when the {{ site.data.dungeon.sbg.carrion_worm }} come back. Try to use them for {{ site.data.spell.splinter }} value.
...
* Use everything at your disposal to survive : {{ site.data.spell.as }}, {{ site.data.spell.ee }}, {{ site.data.spell.ag }}, {{ site.data.item.potion_rhp }}.
...
* Try to spread {{ site.data.spell.fs }} on multiple skeletons {{ site.data.dungeon.sbg.ritual_of_bones }} for {{ site.data.spell.splinter }}, {{ site.data.spell.sfd }} and {{ site.data.spell.lvs }} value.
* It's possible for {{ site.data.spell.lmt }} to apply {{ site.data.spell.fs }} on two skeletons if placed correctly.
...
* Try to line of sight {{ site.data.dungeon.tjs.tainted_ripple }}. Use {{ site.data.spell.as }} if you get the debuff. Use {{ site.data.spell.ag }} to help your party member if any get the debuff.

<hr>

### {{ site.data.dungeon.tjs.wise_mari }}

* Stay far away from the boss to easily drop {{ site.data.dungeon.tjs.corrupted_vortex }}. Use {{ site.data.spell.as }} as needed when dropping it.
* Move around the boss to dodge {{ site.data.dungeon.tjs.wash_away }}. Use {{ site.data.spell.swg }} to keep uptime while moving.
...
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.tjs.haunting_scream }} at all cost. Use {{ site.data.spell.tremor_totem }} if you don't have kicks to spare and a cast is going through.

**{{ site.data.dungeon.tjs.jiang }}** and **{{ site.data.dungeon.tjs.xiang }}**
* You **must** use {{ site.data.spell.as }} when getting the {{ site.data.dungeon.tjs.throw_torch_tjs }} debuff.

**{{ site.data.dungeon.tjs.the_talking_fish }}**
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.tjs.sleepy_soliloquy }}. Use {{ site.data.spell.tremor_totem }} to dispel your party if a cast goes through.

**{{ site.data.dungeon.tjs.lesser_sha }}**
* They will use {{ site.data.dungeon.tjs.sha_eruption }} when dying. Be prepared to use {{ site.data.spell.as }} or {{ site.data.item.potion_rhp }} if you are going to die.
...
* Use hard CC ({{ site.data.spell.capacitor_totem }}) to interrupt {{ site.data.dungeon.tjs.tears_of_pain }}. Dodge the blue swirlies if you don't. Use {{ site.data.spell.swg }} to keep uptime while dodging.
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.tjs.fit_of_rage }}.

**{{ site.data.dungeon.tjs.the_golden_beetle }}**
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.tjs.golden_barrier }}. Be careful, dispelling it will damage the whole group.

**{{ site.data.dungeon.tjs.the_nodding_tiger }}**
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.tjs.cat_nap }}.
* Use {{ site.data.spell.as }} if you get targeted by {{ site.data.dungeon.tjs.savage_leap }}.
...
* {{ site.data.dungeon.tjs.feeling_of_superiority }} is a damage buff received randomly that can be exchanged by going on the current wearer of the buff. Use {{ site.data.spell.as }} when at high stack of {{ site.data.dungeon.tjs.feeling_of_superiority }}.
...
* Use {{ site.data.spell.cleanse_spirit }} to dispel {{ site.data.dungeon.tjs.touch_of_ruin }}.

**{{ site.data.dungeon.tjs.minion_of_doubt }}**
* Dodge the {{ site.data.dungeon.tjs.shattered_resolve }} black swirlies. Use {{ site.data.spell.cleanse_spirit }} to remove the curse it casts on anyone touched.
...
* Dodge the wave of flames from {{ site.data.dungeon.tjs.serpent_kick }} and {{ site.data.dungeon.tjs.jade_serpent_kick }}. Stop casting if you have to. Use {{ site.data.spell.swg }} as needed.
...
* Be ready to use {{ site.data.spell.as }} and {{ site.data.item.potion_rhp }} if your healer didn't dispel {{ site.data.dungeon.tjs.touch_of_nothingness }} on you first.
...
* Dodge {{ site.data.dungeon.av.wild_eruption }} purple swirlies and puddles. Use {{ site.data.spell.swg }} to keep uptime while moving.
...
* {{ site.data.dungeon.av.consuming_stomp }} will deal huge damage, increased by the number of {{ site.data.dungeon.av.ley_line_sprouts }} active. Use {{ site.data.spell.as }} and {{ site.data.spell.ag }} as needed.
...
* You can also use {{ site.data.spell.tremor_totem }} and let some {{ site.data.dungeon.av.waking_bane }} go through.
...
* Dodge the big blue {{ site.data.dungeon.av.forbidden_knowledge }} swirlies. Use {{ site.data.spell.swg }} to keep uptime while dodging.

**{{ site.data.dungeon.av.rune_seal_keeper }}**
* Use {{ site.data.spell.wind_shear }} to interrupt {{ site.data.dungeon.av.icy_bindings }} in priority. Use {{ site.data.spell.thunderous_paws }} to remove the root fast if the cast goes through.
...
* Dodge the blue {{ site.data.dungeon.av.unstable_power }} swirlies. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Dodge the swirlies creates on death by the {{ site.data.dungeon.av.draconic_image }}. But keep {{ site.data.spell.swg }} for the transition.
...
* During {{ site.data.dungeon.av.overwhelming_energy }}, use {{ site.data.spell.swg }} to dodge the {{ site.data.dungeon.av.ancient_orb }} and burst the {{ site.data.dungeon.av.draconic_image }} as fast as possible.
...
* {{ site.data.dungeon.av.bestial_roar }} does a moderate amount of damage to the whole group, be ready to use {{ site.data.spell.as }} and {{ site.data.spell.ag }}.
...
* {{ site.data.dungeon.av.frost_bomb }} will require players to spread and let space for the group to move after it drops. Use {{ site.data.spell.gust_of_wind }} to quickly go out of the patch of ice. Use {{ site.data.spell.swg }} to keep uptime.
* Hide inside of a {{ site.data.dungeon.av.vault_rune }} during {{ site.data.dungeon.av.absolute_zero }}.
* You should already be spreaded for {{ site.data.dungeon.av.icy_devastator }}, just use {{ site.data.spell.as }} if you are targeted.

<hr>

### {{ site.data.dungeon.av.umbrelskul }}

* Try to stay still as much as possible to avoid gaining stacks of {{ site.data.dungeon.av.oppressive_miasma }}. Use {{ site.data.spell.gust_of_wind }} if you need to quickly move and {{ site.data.spell.thunderous_paws }} to reset your stacks.
...
* Watch your feet to dodge {{ site.data.dungeon.no.rain_of_arrows }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.

**{{ site.data.dungeon.no.nokhud_warspear }}**
* They will charge at you and apply {{ site.data.dungeon.no.pierce }}. Consider using {{ site.data.spell.as }} if needed.
...
* Interrupt {{ site.data.dungeon.no.rally_the_clan }} with a hard CC ({{ site.data.spell.capacitor_totem }} or {{ site.data.spell.thunderstorm }}).
...
* Switch on {{ site.data.dungeon.no.nokhud_saboteur }} as soon as it spawn. Use {{ site.data.spell.earthbind_totem }} and {{ site.data.spell.capacitor_totem }} to disrupt it. Do not let him cast {{ site.data.dungeon.no.dismantle }}.
* {{ site.data.dungeon.no.shards_of_stone }} does a lot of party damage. Use {{ site.data.spell.as }} to mitigate and {{ site.data.spell.ag }} to heal the damage.
...
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.no.stormshield }}.
...
* Use {{ site.data.spell.purge }} to dispel {{ site.data.dungeon.no.energy_surge }}.
...
* Use {{ site.data.spell.as }} and {{ site.data.spell.ag }} to survive {{ site.data.dungeon.no.electrical_storm }}.
...
* {{ site.data.dungeon.no.chant_of_the_dead }} makes the tank kite. Be mindful of it when using {{ site.data.spell.eq }} and {{ site.data.spell.lmt }}.

**{{ site.data.dungeon.no.ukhel_beastcaller }}**
* Use hard CC ({{ site.data.spell.capacitor_totem }}) to stop {{ site.data.dungeon.no.desecrating_roar }} or he will spawn 3 additional enemies.
...
* Pick up your sould when targeted by {{ site.data.dungeon.no.shatter_soul }}. Use {{ site.data.spell.swg }} to keep uptime while doing it.
...
* Try to always be above 50% health to not randomly die from {{ site.data.dungeon.no.quick_shot }}. Use {{ site.data.spell.as }} or even {{ site.data.spell.healing_surge }} if needed.
...
* Dodge the brown swirlies during {{ site.data.dungeon.no.earthsplitter }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
* Dodge the brown swirly made by {{ site.data.dungeon.no.ravaging_spear }}. Use {{ site.data.spell.swg }} to keep uptime while dodging.
...
### Phase 1
* When focused by {{ site.data.dungeon.no.iron_spear }}, be a bit away from the boss and use {{ site.data.spell.as }} if needed. Move as soon as the spear hits you to dodge {{ site.data.dungeon.no.iron_stampede }}.
...
* Assign a {{ site.data.dungeon.no.nokhud_stormcaster }} to kick per person and quickly pack them. Depending on your group, you might be asked to {{ site.data.spell.hex }} one of them.
* Dodge lightning blue swirlies during the phase.

### Phase 2
* The whole group will be pulled on the position of the target of {{ site.data.dungeon.no.static_spear }}. If targeted, be a bit away from the boss but don't pull your party into the wall. Use {{ site.data.spell.as }} if needed. Move as soon as the spear hits to dodge {{ site.data.dungeon.no.iron_stampede }}.
